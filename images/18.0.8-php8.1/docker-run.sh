...
function initializeDatabase() {
	...
	if [[ ${DOLI_INIT_DEMO} -eq 1 ]]; then
		...
		versiondemo=$(echo "${DOLI_TAG}" | sed "s/^\([0-9]*\.[0-9]*\).*/\1.0/") # Convert version x.y.z into x.y.0
...
function migrateDatabase() {
	...
	truncate -s 0 /var/www/documents/migration_error.html
...
function run() {
	truncate -s 0 /var/www/documents/initdb.log 2>&1
